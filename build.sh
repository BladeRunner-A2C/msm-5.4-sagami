...
ZIPNAME="QuicksilveR-sagami-$(date '+%Y%m%d-%H%M').zip"
...
make -j$(nproc --all) $MAKE_PARAMS INSTALL_MOD_PATH=modules INSTALL_MOD_STRIP=1 modules_install 2> >(tee error.log >&2)

kernel="$OUT_DIR/arch/arm64/boot/Image"
dtb_dir="$OUT_DIR/arch/arm64/boot/dts/vendor/qcom"
dtbo_dir="$OUT_DIR/arch/arm64/boot/dts/vendor/somc"
